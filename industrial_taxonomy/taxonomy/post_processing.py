...
if __name__ == "__main__":
    ...
    high_homog = set(homog.query("median_similarity>0.5")["index"].tolist())
    ...
    if os.path.exists(D2V_PATH) is False:
        ...
        os.logging("Loading d2v model")
        d2v = load_model("d2v")

    label_average_vectors = make_average_docvecs(labelled_set)

    # Labelled dataset with closest
    # Unlabelled dataset
    unl = g_lab.query("label_2=='unlabelled'").reset_index(drop=True)
    ...
    closest_sector_df = pd.DataFrame(
        find_closest_sector(
            unl.iloc[:10], d2v, columns=["sector", "similarity"]
        ).assign(org_id=unl["org_id"])
    )

    save_table(closest_sector_df, "unlabelled_closest_sector_TEST")

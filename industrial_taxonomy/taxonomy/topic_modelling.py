...
import pandas as pd
from industrial_taxonomy.hSBM_Topicmodel.sbmtm import sbmtm
...
def post_process_model_clusters(
    model, top_level, cl_level, top_thres=1, top_words=5, n="all", prefix=""
):
    """Function to post-process the outputs of a hierarchical topic model
    _____
    Args:
        ...
        prefix: prefix to laabel the clusters
    ...
    id_cluster_lookup = {
        int(el[0]): prefix + str(k) for k, v in cluster_assignment.items() for el in v
    }
    ...
    return topic_mix, id_cluster_lookup
